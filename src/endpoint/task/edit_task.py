...
def edit_task():
    ...
    try:
        ...
        if new_title is not None:
            cursor.execute(
                "UPDATE tasks SET title = %s WHERE title = %s AND user_id = %s;",
        ...
        if new_description is not None:
            cursor.execute(
                "UPDATE tasks SET description = %s WHERE description = %s AND user_id = %s;",
                (new_description, description, user_id),
            )
            description = new_description

        if new_status is not None:
            cursor.execute(
                "UPDATE tasks SET status = %s WHERE status = %s AND user_id = %s;",
                (new_status, status, user_id),
            )
            status = new_status

        if new_priority is not None:
            cursor.execute(
                "UPDATE tasks SET priority = %s WHERE priority = %s AND user_id = %s;",
                (new_priority, priority, user_id),
            )
